function updateNotificationCount(count) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const dateSpan = document.getElementById("registrar_clearance_dateToday");
...
document.getElementById("registrar_clearance_otpSidebar").classList.remove("show");
}
...
const buttons = document.querySelectorAll(".registrar-clearance-action-btn");
...
buttons.forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if (action === "Approved") {
      ...
      document.getElementById("registrar_clearance_otpSidebar").classList.add("show");
    } else {
      currentDisapproveId = id;
      clearDisapproveChecks();
      document.getElementById("registrar_clearance_disapproveError").style.display = "none";
      document.getElementById("registrar_clearance_appointmentError").style.display = "none";
      closeOtpSidebar();
      document.getElementById("registrar_clearance_disapproveSidebar").classList.add("show");
    }
  });
});

// OTP verification
document.getElementById("registrar_clearance_verifyOtpBtn").addEventListener("click", () => {
  const input = document.getElementById("registrar_clearance_otpinput");
  const code = input.value.trim();
  const correctCode = "12345678";
  if (code === correctCode) {
    handleApproval("Approved", `registrar_clearance_timestamp_registrar_${currentApproveId}`);
    closeOtpSidebar();
  } else {
...
document.getElementById("registrar_clearance_submit_Appointment_Disapproval_Btn").addEventListener("click", () => {
  let checkedReasons = Array.from(document.querySelectorAll('#registrar_clearance_disapproveSidebar input[type="checkbox"]:checked')).map(cb => cb.value);
  const appointmentDate = document.getElementById("registrar_clearance_appointmentDate").value;

  const otherReasonChecked = document.getElementById("registrar_clearance_reasonOther").checked;
  const otherReasonInput = document.getElementById("registrar_clearance_otherReasonInput").value.trim();
  ...
  const target = document.getElementById(`registrar_clearance_timestamp_registrar_${currentDisapproveId}`);
  if (target) {
    ...
    const comment = document.getElementById("registrar_clearance_disapproveComment").value.trim();
    if (comment) {
...
document.getElementById("registrar_clearance_reasonOther").addEventListener("change", toggleOtherReasonInput);
...
document.getElementById("registrar_clearance_disapproveSidebar").classList.remove("show");
}

function clearDisapproveChecks() {
document.getElementById("registrar_clearance_disapproveComment").value = "";
document.querySelectorAll('#registrar_clearance_disapproveSidebar input[type="checkbox"]').forEach(cb => cb.checked = false);
...
function handleApproval(action, timestampId) {
  ...
  if (el) {
    ...
    const comment = document.getElementById("registrar_clearance_otpComment").value.trim(); 
...
const otherInputContainer = document.getElementById("registrar_clearance_otherReasonContainer");
