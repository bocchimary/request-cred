...
document.addEventListener("DOMContentLoaded", function () {
  const dateSpan = document.getElementById("signatory_enrollment_dateToday");
...
document.getElementById("signatory_enrollment_otpSidebar").classList.remove("show");
}
...
const buttons = document.querySelectorAll(".signatory-enrollment-action-btn");
...
buttons.forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if (action === "Approved") {
      ...
      document.getElementById("signatory_enrollment_otpSidebar").classList.add("show");
    } else {
      ...
      closeOtpSidebar();
      document.getElementById("signatory_enrollment_disapproveSidebar").classList.add("show");
    }
  });
});

// OTP verification
document.getElementById("signatory_enrollment_verifyOtpBtn").addEventListener("click", () => {
  const input = document.getElementById("signatory_enrollment_otpinput");
  const code = input.value.trim();
  const correctCode = "12345678";
  if (code === correctCode) {
    handleApproval("Approved", `signatory_enrollment_timestamp_signatory_${currentApproveId}`);
    closeOtpSidebar();
  } else {
    document.getElementById("signatory_enrollment_otpError").style.display = "block";
  }
});

// Submit Disapproval with appointment
document.getElementById("signatory_enrollment_submit_Appointment_Disapproval_Btn").addEventListener("click", () => {
  let checkedReasons = Array.from(document.querySelectorAll('#signatory_enrollment_disapproveSidebar input[type="checkbox"]:checked')).map(cb => cb.value);
  const appointmentDate = document.getElementById("signatory_enrollment_appointmentDate").value;

  const otherReasonChecked = document.getElementById("signatory_enrollment_reasonOther").checked;
  const otherReasonInput = document.getElementById("signatory_enrollment_otherReasonInput").value.trim();
  ...
  if (checkedReasons.length === 0) {
    document.getElementById("signatory_enrollment_disapproveError").style.display = "block";
    return;
  }
  if (!appointmentDate) {
    document.getElementById("signatory_enrollment_appointmentError").style.display = "block";
    return;
  }
  ...
  const target = document.getElementById(`signatory_enrollment_timestamp_signatory_${currentDisapproveId}`);
  if (target) {
    ...
    const comment = document.getElementById("signatory_enrollment_disapproveComment").value.trim();
    if (comment) {
...
document.getElementById("signatory_enrollment_reasonOther").addEventListener("change", toggleOtherReasonInput);
...
document.getElementById("signatory_enrollment_disapproveSidebar").classList.remove("show");
}

function clearDisapproveChecks() {
document.getElementById("signatory_enrollment_disapproveComment").value = "";
document.querySelectorAll('#signatory_enrollment_disapproveSidebar input[type="checkbox"]').forEach(cb => cb.checked = false);
...
document.getElementById("signatory_enrollment_otherReasonContainer").style.display = "none";
}
...
function handleApproval(action, timestampId) {
  ...
  if (el) {
    ...
    const comment = document.getElementById("signatory_enrollment_otpComment").value.trim(); 
...
const otherInputContainer = document.getElementById("signatory_enrollment_otherReasonContainer");
