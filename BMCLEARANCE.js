...
document.addEventListener("DOMContentLoaded", function () {
  const dateSpan = document.getElementById("bm_clearance_dateToday");
...
const buttons = document.querySelectorAll(".bm-clearance-action-btn");
...
// OTP verification
document.getElementById("bm_clearance_verifyOtpBtn").addEventListener("click", () => {
  const input = document.getElementById("bm_clearance_otpinput");
  const code = input.value.trim();
  const correctCode = "12345678";
  if (code === correctCode) {
    handleApproval("Approved", `bm_clearance_timestamp_bm_${currentApproveId}`);
    closeOtpSidebar();
  } else {
...
document.getElementById("bm_clearance_submit_Appointment_Disapproval_Btn").addEventListener("click", () => {
  ...
  const target = document.getElementById(`bm_clearance_timestamp_bm_${currentDisapproveId}`);
  if (target) {
...
document.getElementById("bm_clearance_reasonOther").addEventListener("change", toggleOtherReasonInput);
...
const otherInputContainer = document.getElementById("bm_clearance_otherReasonContainer");
