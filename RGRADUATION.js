...
// OTP verification
document.getElementById("registrar_graduation_verifyOtpBtn").addEventListener("click", () => {
  const input = document.getElementById("registrar_graduation_otpinput");
  const code = input.value.trim();
  const correctCode = "12345678";
  if (code === correctCode) {
    handleApproval("Approved", `registrar_graduation_timestamp_registrar_${currentApproveId}`);
    closeOtpSidebar();
  } else {
...
document.getElementById("registrar_graduation_submit_Appointment_Disapproval_Btn").addEventListener("click", () => {
  ...
  const target = document.getElementById(`registrar_graduation_timestamp_registrar_${currentDisapproveId}`);
  if (target) {
