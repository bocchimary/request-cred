...
const buttons = document.querySelectorAll(".bm-cred_req-action-btn");
...
// OTP verification
document.getElementById("bm_cred_req_verifyOtpBtn").addEventListener("click", () => {
  const input = document.getElementById("bm_cred_req_otpinput");
  const code = input.value.trim();
  const correctCode = "12345678";
  if (code === correctCode) {
    handleApproval("Approved", `bm_cred_req_timestamp_bm_${currentApproveId}`);
    closeOtpSidebar();
  } else {
...
document.getElementById("bm_cred_req_submit_Appointment_Disapproval_Btn").addEventListener("click", () => {
  let checkedReasons = Array.from(document.querySelectorAll('#bm_cred_req_disapproveSidebar input[type="checkbox"]:checked')).map(cb => cb.value);
  const appointmentDate = document.getElementById("bm_cred_req_appointmentDate").value;

  const otherReasonChecked = document.getElementById("bm_cred_req_reasonOther").checked;
  const otherReasonInput = document.getElementById("bm_cred_req_otherReasonInput").value.trim();
  ...
  const target = document.getElementById(`bm_cred_req_timestamp_bm_${currentDisapproveId}`);
  if (target) {
...
document.getElementById("bm_cred_req_reasonOther").addEventListener("change", toggleOtherReasonInput);
...
const otherInputContainer = document.getElementById("bm_cred_req  _otherReasonContainer");
